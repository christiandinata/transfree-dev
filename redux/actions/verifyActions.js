...
const verify = ({ phone, email, fullname, password }, type) => {
	...
	return (dispatch) => {
		...
		axios
			.post(`${API}/v1/${type}`, { email, phone })
			// axios.post(`${API}/login`, { email, password })
			.then((response) => {
				console.log(response);
				dispatch({
					type: VERIFY_PHONE,
					payload: response.data.serviceSid,
				});
				dispatch({
					type: INITIAL_DATA_USER,
					payload: { email, fullname, password, phone },
				});
