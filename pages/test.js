...
class EditProfile extends React.Component {
	...
	validateData = () => {
		if (!GlobalFunction.validateEmail(this.state.emailUser)) {
			...
			this.setState({
				dobError: "Please insert your DOB (Date of birth) !",
			});
			return false;
		} else if (!this.state.pob) {
			this.setState({
				pobError: "Please insert your POB (Place of birth) !",
			});
			return false;
			...
			this.setState({
				verificationCodeError: "Please insert verification code !",
			});
			return false;
		} else {
			return true;
	...
	handleSubmit = (event) => {
		...
		Axios.put(urlFetch, { user }).then((res) => {
			console.log(res);
...
const mapDispatchToProps = (dispatch) => ({
	...
	onChangeUserPasswordLogin: (value) =>
		dispatch(onChangeUserPasswordLogin(value)),
});
